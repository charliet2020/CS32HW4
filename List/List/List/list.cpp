...
{
	if (m->menuItems() == nullptr) //if plainitem, print out with path, then stop
	{
		string compPlain;
		compPlain = path + m->name();
		//cerr << "plainitem, ";
		cout << compPlain << endl;
		return;
	}
	...
	/**/
	if (m->name()!= "") //we wanna print out current name, 
	{
		cerr << "name is not blank, ";
	...
	while (it != m->menuItems()->end())
	{
		...
		if (m->name() != "")
			temp = path + m->name() + '/';
		listAll(*it, temp); //temp will either keep its string or have more stuff added
		//will iterate through vector once we hit a plainItem
		it++;
	}
